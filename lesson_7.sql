...
-- Получить название бренда у которого идентификатор = 3
SELECT * FROM brand WHERE id = 3;

-- Добавляем данные в таблицу product_type
INSERT INTO product_type (name) VALUE ('Платье');
INSERT INTO product_type (name) VALUE ('Футболка');
INSERT INTO product_type (name) VALUE ('Свитер');

-- Получить первые два типа товара из таблиы product_type
SELECT * FROM product_type LIMIT 2;

-- Получить все категории товаров со скидкой < 10%, и отсортировать их по названию
SELECT * FROM category WHERE discount < 10 ORDER BY category.name ASC;